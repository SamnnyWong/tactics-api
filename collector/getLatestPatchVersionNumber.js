...
export const main = handler(async (event, context, callback) => {
    ...
    if (!githubCommitMessages || !result) throw Error('###Tactics Log###: Fetching commit message failed: Service Terminating...');
    ...
    if (!commitMessage || !latestVersionNumber) throw Error('###Tactics Log###: Can not get version number from commit message: Service Terminating...');
    ...
    if (!scanDBResponse) throw Error('###Tactics Log###: Can not get PVH record: Service Terminating...');
    ...
    if (currentPVHVersionNUmber == latestVersionNumber) {
        ...
        if (! putDBResponse) throw Error('###Tactics Log###: Putting in DB failed, service terminating...');
